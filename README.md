...
прогоняются автоматически в `tests/test_audio_fixtures.py` и проверяют коды
400/422 и ветку пустого транскрипта.
...
- Речь **синтетическая**: нет фонового шума, перебиваний и наложения голосов,
  дикция чище реальных звонков. Абсолютные значения WER здесь — оптимистичная
  оценка, а не прогноз для продакшена.
- **RTF 0.63 на CPU не укладывается в требование «отклик < 60 сек на файл до
